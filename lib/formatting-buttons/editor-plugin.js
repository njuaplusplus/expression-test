...
var pexetoButtons=[
		{
			...
			generateHtml:function(text){
				return '<h1 class="page-heading">'+text+'</h1>';
			}
		},
        {
			id:'pexetohighlight1',
			...
			if(btn.fields[i].values){
				...
				if(btn.fields[i].textarea && !pexetoButtonManager.opera){
					...
					if(btn.fields[i].type==='upload'){
						inputHtml+='<input type="button" class="pexeto-upload-btn button-secondary" value="Upload Image" />';
					}
				}
			}
