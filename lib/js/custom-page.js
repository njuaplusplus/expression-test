	...
	function setEditButtonClickHandlers(){
		$root.delegate('.edit-button', 'click', function(){
			...
			$parentLi.find(options.valueContainer).each(function(){
				...
				if(isTextarea){
					$that.replaceWith($('<textarea />', {name:itemClasses[0], value:$that.html(), 'class':$that.attr('class')}));
				}else{
					$that.replaceWith($('<input />', {name:itemClasses[0], value:$that.html(), type:'text', 'class':$that.attr('class')}));
				}
			});
